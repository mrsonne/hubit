...
def skipfun(value_for_path: Dict) -> bool:
    ...
    ref = seg_thcks[0]
    not_all_eq_tck = not all( [isclose(ref, seg_thck, rel_tol=1e-5) 
...
def make_sweep(hmodel: HubitModel, nproc:Any=None) -> None:
    ...
    input_values_for_path = {
                             ...
                             "segments[0].layers[2].thickness": (0.08, 0.12,), # insulation thickness
                             "segments[2].layers[1].thickness": (0.025, 0.065,),
                            }
    ...
    float_formats = [q_float_formats[header] if header in q_float_formats else '' 
    ...
    lines.append( sepstr )
    fstr = ''.join( [f'{{:<{width}{float_format}}}' 
